...
void mb_update_odometry(mb_odometry_t* mb_odometry, mb_state_t* mb_state){
    ...
    float enc2meters = (WHEEL_DIAMETER * PI) / (GEAR_RATIO * ENCODER_RES / 2);
    ...
    // publish new odometry to lcm ODOMETRY_CHANNEL
    // odometry_t current_odometry;
    // current_odometry.x = mb_odometry->x;
    // current_odometry.y = mb_odometry->y;
    // current_odometry.theta = mb_odometry->theta;
    // odometry_t_publish(lcm, ODOMETRY_CHANNEL, &current_odometry);
}
