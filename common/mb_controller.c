...
int mb_controller_update(mb_state_t* mb_state, mb_setpoints_t* mb_setpoints){
    ...
    int ratio_ = 2.5; //2.5
    float jump = 0.01;
    mb_state->left_cmd = (ratio_*left_sp + jump * ((left_sp<0)?-1:1)) + rc_filter_march(&left_pid, left_sp - mb_state->left_velocity);
    mb_state->right_cmd =(ratio_*right_sp + jump * ((right_sp<0)?-1:1)) + rc_filter_march(&right_pid, right_sp - mb_state->right_velocity);
